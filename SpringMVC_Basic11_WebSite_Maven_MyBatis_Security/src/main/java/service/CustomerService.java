...
public class CustomerService {
	...
	public String noticeReg(Notice n, HttpServletRequest request, Principal principal) throws Exception {
		...
		//인증된 사용자 정보를 가지고 와서 
		//n.setWriter(null) 같은 작업을 할 건데 이건 2차 프로젝트 때 했던
		//session.getAttribute("id") 와 같은 작업이다
		//spring이 위 작업을 해주기 때문에 우리는 spring이 가진 자원을 끌고 와서 사용할 수 있다
		SecurityContext context = SecurityContextHolder.getContext(); //스프링 시큐리티의 정보 중에
		Authentication auth = context.getAuthentication(); // 인증에 관련된 것만
		
		// UserDetails는 로그인한 유저의 정보를 담은 객체
		UserDetails userinfo = (UserDetails)auth.getPrincipal();
									// Object 타입이기 때문에 캐스팅 필수
		...
		//Principal principal - 인증되면 인증 객체 정보를 받는다(스프링)
